...
function home(){
 ...
 return `<section class="page">
  ...
  <div class="card studygoalpanel">
   ...
   ${activeGoals.map(g=>`<div class="goalmini" data-id="${g.id}"><div class="grow"><strong>🎯 ${esc(g.title)}</strong><span>${esc(g.target||"Keep going")}</span></div><button class="inlinefocus" onclick="openFocus('goal','${g.id}')">⏱ Focus</button><button class="chk goalcheck" aria-label="Mark goal complete" onclick="toggleGoal('${g.id}')"><span class="burst"></span><svg viewBox="0 0 24 24"><path d="M5 12l4.5 4.5L19 7.5"/></svg></button></div>`).join("")}
   ${doneGoals.map(g=>`<div class="goalmini done" data-id="${g.id}"><div class="grow"><strong>✓ ${esc(g.title)}</strong><span>Completed${g.target?" · "+esc(g.target):""}</span></div><button class="chk on goalcheck" aria-label="Mark goal active" onclick="toggleGoal('${g.id}')"><span class="burst"></span><svg viewBox="0 0 24 24"><path d="M5 12l4.5 4.5L19 7.5"/></svg></button></div>`).join("")}
...
function playCompletionEffect(btn,card,message="Nice! Marked as done ✓",undo){if(!btn)return;btn.classList.add("on","anim");card?.classList.add("done","popping");confetti(btn);try{navigator.vibrate&&navigator.vibrate([14,50,22])}catch(x){}if(message)toast(message,undo);setTimeout(()=>{btn.classList.remove("anim");card?.classList.remove("popping")},950)}
window.toggleGoal=id=>{let g=data.goals.find(x=>x.id===id);if(!g)return;const was=g.done;g.done=!was;save();closeModal();openGoals();requestAnimationFrame(()=>{const row=[...document.querySelectorAll(".goalmini")].find(x=>x.dataset.id===id),b=row?.querySelector(".goalcheck");if(!b)return;if(!was)playCompletionEffect(b,row,"Nice! Goal marked as done ✓",()=>{g.done=false;save();closeModal();openGoals()});else{b.classList.remove("on");row.classList.remove("done");b.classList.add("anim");row.classList.add("popping");setTimeout(()=>{b.classList.remove("anim");row.classList.remove("popping")},950)}})};
...
function eventTargetMs(e){if(!e?.date)return 0;return new Date(`${e.date}T${e.time||"00:00"}:00`).getTime()}
function formatEventCountdown(e){const ms=eventTargetMs(e)-Date.now();if(ms<=0)return "Happening now";let sec=Math.floor(ms/1000),days=Math.floor(sec/86400);sec%=86400;let h=Math.floor(sec/3600);sec%=3600;let m=Math.floor(sec/60),s=sec%60;if(days)return `${days}d ${String(h).padStart(2,"0")}h`;if(h)return `${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}:${String(s).padStart(2,"0")}`;return `${String(m).padStart(2,"0")}:${String(s).padStart(2,"0")}`}
function renderEventCountdowns(){document.querySelectorAll(".eventcountdown").forEach(el=>{const e=data.events.find(x=>x.id===el.dataset.eventId);if(e)el.textContent=formatEventCountdown(e)})}
setInterval(renderEventCountdowns,1000);
...
window.toggleDone=(id,btn)=>{
 ...
 const card=btn.closest(".eventcard")||btn.closest(".homeevent");
 if(!was)playCompletionEffect(btn,card,"Nice! Marked as done ✓",()=>{e.status="Upcoming";e.doneAt=null;save();shell()});

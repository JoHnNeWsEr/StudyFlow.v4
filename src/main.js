...
function home(){
 ...
 return `<section class="page">
  ...
  <div class="statgrid"><button class="stat" onclick="go('events')"><b>${data.events.filter(e=>e.status!=="Completed").length}</b><span>Upcoming</span></button><button class="stat" onclick="openGoals()"><b>${data.goals.filter(g=>!g.done).length}</b><span>Goals left</span></button><button class="stat" onclick="openGrades()"><b>${data.grades.length}</b><span>Grades</span></button></div>
  ${data.goals.filter(g=>!g.done).slice(0,2).map(g=>`<div class="card goalmini"><div class="grow"><strong>🎯 ${esc(g.title)}</strong><span>${esc(g.target||"Keep going")}</span></div><button onclick="toggleGoal('${g.id}')">Done</button></div>`).join("")}
...
let focusTimer=null,focusLeft=25*60;
window.openFocus=()=>{clearInterval(focusTimer);focusLeft=25*60;modal("Focus timer",`<div class="focus"><div id="focusclock">25:00</div><p class="muted">25-minute focus session · then take a 5-minute break.</p><button class="primary wide" onclick="startFocus()">Start</button><button class="wide" onclick="resetFocus()">Reset</button></div>`)};
window.startFocus=()=>{if(focusTimer)return;focusTimer=setInterval(()=>{focusLeft--;let el=document.querySelector('#focusclock');if(el)el.textContent=Math.floor(focusLeft/60).toString().padStart(2,'0')+':'+(focusLeft%60).toString().padStart(2,'0');if(focusLeft<=0){clearInterval(focusTimer);focusTimer=null;alert('Focus session complete! Take a 5-minute break 🎉');}},1000)};
window.resetFocus=()=>{clearInterval(focusTimer);focusTimer=null;focusLeft=25*60;let el=document.querySelector('#focusclock');if(el)el.textContent='25:00'};

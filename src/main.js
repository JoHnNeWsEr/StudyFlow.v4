...
function home(){
 ...
 const goalMarkup=goalRows.map(({g,done})=>{
  ...
  const liveCount=activeFocus?`<button type="button" class="focusrunning" onclick="openGoalFocusControls('${g.id}')" aria-label="Focus controls for ${esc(g.title)}"><span class="focusdot"></span><span class="focusvalue">${formatFocus(Math.max(0,Math.ceil((liveEndAt-Date.now())/1000)))}</span></button>`:"";
  return `<div class="goalmini ${done?"done":""}" data-id="${g.id}"><div class="grow goalinline"><div class="goaltitlerow"><strong>${done?"✓":"🎯"} ${esc(g.title)}</strong></div>${done?`<span>Completed${g.target?" · "+esc(g.target):""}</span>`:esc(g.target||"Keep going")}</div>${liveCount}<button class="chk goalcheck ${done?"on":""}" aria-label="${done?"Mark goal active":"Mark goal complete"}" onclick="toggleGoal('${g.id}',this)"><span class="burst"></span><svg viewBox="0 0 24 24"><path d="M5 12l4.5 4.5L19 7.5"/></svg></button></div>`;
 }).join("");
...
window.addGoal=()=>modal("New study goal",`<form onsubmit="saveGoal(event)"><label>Goal<input id="gotitle" required placeholder="Finish Chapter 3"></label><label>Target / detail<input id="gotarget" placeholder="By Friday"></label><div class="row"><label>Focus duration<input id="goduration" type="number" min="1" step="1" value="25"></label><label>Unit<select id="gounit"><option value="seconds">Seconds</option><option value="minutes" selected>Minutes</option><option value="hours">Hours</option></select></label></div><p class="muted goalhint">Start this goal later from its ⋯ menu. Starting it there begins the countdown immediately.</p><button class="primary wide">Save goal</button></form>`);
window.saveGoal=e=>{e.preventDefault();const title=document.querySelector("#gotitle")?.value.trim();const target=document.querySelector("#gotarget")?.value.trim()||"";const n=Math.max(1,Number(document.querySelector("#goduration")?.value||25));const u=document.querySelector("#gounit")?.value||"minutes";const mult=u==="hours"?3600:u==="seconds"?1:60;if(!title)return;data.goals.push({id:uid(),title,target,done:false,focusDuration:Math.round(n*mult)});save();closeModal();openGoals()};
...
window.goalMenu=id=>{const g=data.goals.find(x=>x.id===id);if(!g||g.done)return;const active=focusEndAt&&focusTarget?.type==="goal"&&focusTarget.id===id;modal("Goal options",`<div class="goalactions"><button class="primary wide" onclick="startGoalFocus('${id}')">${active?"▶ Running…":"▶ Start"}</button><button class="wide" onclick="editGoalDuration('${id}')">⏱ Edit duration</button><button class="delete wide" onclick="deleteGoal('${id}')">Delete</button></div>`) };
window.editGoalDuration=id=>{const g=data.goals.find(x=>x.id===id);if(!g)return;const sec=Math.max(1,Number(g.focusDuration)||25*60);const unit=sec%3600===0?"hours":sec%60===0?"minutes":"seconds";const amount=unit==="hours"?sec/3600:unit==="minutes"?sec/60:sec;modal("Edit duration",`<form onsubmit="saveGoalDuration(event,'${id}')"><p class="muted">${esc(g.title)}</p><div class="row"><label>Duration<input id="goduration" type="number" min="1" step="1" value="${amount}"></label><label>Unit<select id="gounit"><option value="seconds" ${unit==="seconds"?"selected":""}>Seconds</option><option value="minutes" ${unit==="minutes"?"selected":""}>Minutes</option><option value="hours" ${unit==="hours"?"selected":""}>Hours</option></select></label></div><button class="primary wide">Save duration</button></form>`) };
window.saveGoalDuration=(e,id)=>{e.preventDefault();const g=data.goals.find(x=>x.id===id);if(!g)return;const n=Math.max(1,Number(document.querySelector("#goduration")?.value||25));const u=document.querySelector("#gounit")?.value||"minutes";const mult=u==="hours"?3600:u==="seconds"?1:60;g.focusDuration=Math.round(n*mult);save();closeModal();openGoals()};
window.startGoalFocus=async id=>{const g=data.goals.find(x=>x.id===id);if(!g||g.done)return;const sec=Math.max(1,Number(g.focusDuration)||25*60);focusTarget={type:"goal",id};focusTotal=sec;focusLeft=sec;focusEndAt=Date.now()+sec*1000;writeFocus();reconcileFocus();closeModal();shell();requestAnimationFrame(()=>{reconcileFocus();renderFocus()});scheduleFocusNotification().catch(e=>console.warn("Focus notification failed",e))};
window.openGoalFocusControls=id=>{const g=data.goals.find(x=>x.id===id);if(!g||g.done)return;const active=!!(focusEndAt&&focusTarget?.type==="goal"&&focusTarget.id===id);const current=active?formatFocus(focusLeft):formatFocus(Math.max(0,Number(g.focusDuration)||25*60));modal("Focus controls",`<div class="focus"><div class="focustarget">🎯 <strong>${esc(g.title)}</strong></div><div id="goalcontrolclock">${current}</div><p class="muted">${active?"This goal is currently running.":"This goal is not running."}</p><button class="wide" onclick="editGoalDuration('${id}')">⏱ Edit duration</button><button class="wide" onclick="resetGoalFocus('${id}')">↺ Reset</button><button class="primary wide" onclick="startGoalFocus('${id}')">▶ Start</button></div>`)};
window.resetGoalFocus=async id=>{const g=data.goals.find(x=>x.id===id);if(!g)return;clearInterval(focusTimer);focusTimer=null;focusEndAt=null;focusTarget={type:"goal",id};focusTotal=Math.max(1,Number(g.focusDuration)||25*60);focusLeft=focusTotal;writeFocus();await cancelFocusNotification();closeModal();shell();requestAnimationFrame(()=>renderFocus())};
window.deleteGoal=id=>{data.goals=data.goals.filter(x=>x.id!==id);save();closeModal();openGoals()};
...
const STEPS=[
 {t:"Welcome to StudyFlow 👋",d:"This guide is made for a brand-new StudyFlow setup. We’ll show you where to create your first subject, class, event and study goal."},
 {v:"home",sel:".hgrid",place:"header-below",t:"Your StudyFlow header",d:"This is your profile and quick-add area. The guide highlights the header and keeps this message underneath it so nothing is covered."},
 ...
 {t:"You’re ready! 🎉",d:"Start by adding your first subject and class. You can replay this tutorial anytime from Settings → Replay tutorial."}
];
let tourI=0;

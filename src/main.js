...
window.addGoal=()=>modal("New study goal",`<form onsubmit="saveGoal(event)"><label>Goal<input id="gotitle" required placeholder="Finish Chapter 3"></label><label>Target / detail<input id="gotarget" placeholder="By Friday"></label><div class="row"><label>Focus duration<input id="goduration" type="number" min="1" step="1" value="25" required></label><label>Unit<select id="gounit"><option value="seconds">Seconds</option><option value="minutes" selected>Minutes</option><option value="hours">Hours</option></select></label></div><p class="muted goalstartnote">Set the focus duration now, then choose Start now if you want to begin immediately.</p><div class="row goalformactions"><button type="button" class="wide" onclick="saveGoal(event,false)">Save goal</button><button type="submit" name="goalaction" value="start" class="primary wide">Start now</button></div></form>`);
window.saveGoal=(e,startOverride=null)=>{if(e?.preventDefault)e.preventDefault();const title=document.querySelector("#gotitle")?.value.trim();const target=document.querySelector("#gotarget")?.value.trim()||"";if(!title)return;const n=Math.max(1,Number(document.querySelector("#goduration")?.value||25));const u=document.querySelector("#gounit")?.value||"minutes";const mult=u==="hours"?3600:u==="seconds"?1:60;const seconds=Math.round(n*mult);const id=uid();data.goals.push({id,title,target,done:false});save();const startNow=startOverride===true||e?.submitter?.value==="start";if(!startNow){closeModal();openGoals();return;}focusTarget={type:"goal",id};focusTotal=seconds;focusLeft=seconds;focusEndAt=Date.now()+seconds*1000;writeFocus();reconcileFocus();closeModal();shell();requestAnimationFrame(()=>{reconcileFocus();renderFocus();});scheduleFocusNotification().catch(err=>console.warn("Focus notification failed",err));};

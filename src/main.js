...
function home(){
 ...
 const goalMarkup=goalRows.map(({g,done})=>{
  ...
  return `<div class="goalmini ${done?"done":""}" data-id="${g.id}"><div class="grow goalinline"><div class="goaltitlerow"><strong>${done?"✓":"🎯"} ${esc(g.title)}</strong>${!done&&activeFocus?`<span class="focusrunning"><span class="focusdot"></span><span class="focusvalue">${formatFocus(Math.max(0,Math.ceil((liveEndAt-Date.now())/1000)))}</span></span>`:""}</div>${done?`<span>Completed${g.target?" · "+esc(g.target):""}</span>`:esc(g.target||"Keep going")}</div><button class="chk goalcheck ${done?"on":""}" aria-label="${done?"Mark goal active":"Mark goal complete"}" onclick="toggleGoal('${g.id}',this)"><span class="burst"></span><svg viewBox="0 0 24 24"><path d="M5 12l4.5 4.5L19 7.5"/></svg></button></div>`;
 }).join("");

...
function home(){
 ...
 return `<section class="page">
  ...
  <div class="card studygoalpanel">
   ...
   ${activeGoals.map(g=>`<div class="goalmini" data-id="${g.id}"><div class="grow"><strong>🎯 ${esc(g.title)}</strong><span>${esc(g.target||"Keep going")}</span></div><button class="inlinefocus" onclick="openFocus('goal','${g.id}')">⏱ Focus</button><button class="complete" onclick="toggleGoal('${g.id}')">Done</button></div>`).join("")}
...
async function cancelFocusNotification(){if(!native)return;try{await LocalNotifications.cancel({notifications:[{id:FOCUS_NOTIFY_ID}]})}catch(e){}}
...
function renderFocus(){let el=document.querySelector("#focusclock");if(el)el.textContent=formatFocus(focusLeft);let btn=document.querySelector("#focusstart");if(btn)btn.textContent=focusEndAt?"Running…":"Start";document.querySelectorAll(".inlinefocus").forEach(b=>{let row=b.closest(".goalmini")||b.closest(".eventcard"),type=b.closest(".goalmini")?"goal":"event",id=row?.dataset.id,active=focusEndAt&&focusTarget?.type===type&&focusTarget.id===id;b.textContent=active?`⏱ ${formatFocus(focusLeft)}`:(type==="goal"?"⏱ Focus":"⏱");b.classList.toggle("running",!!active)})}

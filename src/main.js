...
function home(){
 ...
 return `<section class="page">
  ...
  <div class="card studyfocus"><div class="grow"><strong>🎯 Goals</strong><span>${activeGoals.length?activeGoals.length+" active goal"+(activeGoals.length===1?"":"s"):"No active goals"}</span></div><button class="primary" onclick="openFocus()">⏱ Focus</button></div>
  ${activeGoals.map(g=>`<div class="card goalmini"><div class="grow"><strong>🎯 ${esc(g.title)}</strong><span>${esc(g.target||"Keep going")}</span></div><button onclick="toggleGoal('${g.id}')">Done</button></div>`).join("")}
  ${doneGoals.map(g=>`<div class="card goalmini done"><div class="grow"><strong>✓ ${esc(g.title)}</strong><span>Completed${g.target?" · "+esc(g.target):""}</span></div><button onclick="toggleGoal('${g.id}')">Undo</button></div>`).join("")}
...
let focusTimer=null,focusLeft=25*60,focusTotal=25*60;
...
window.openFocus=()=>{clearInterval(focusTimer);focusTimer=null;focusLeft=25*60;focusTotal=25*60;modal("Focus timer",`<div class="focus"><div id="focusclock">${formatFocus(focusLeft)}</div><p class="muted">Choose any duration in seconds, minutes, or hours.</p><div class="row"><label>Duration<input id="focusamount" type="number" min="1" step="1" value="25"></label><label>Unit<select id="focusunit"><option value="seconds">Seconds</option><option value="minutes" selected>Minutes</option><option value="hours">Hours</option></select></label></div><button class="wide" onclick="setFocusDuration()">Set duration</button><button class="primary wide" onclick="startFocus()">Start</button><button class="wide" onclick="resetFocus()">Reset</button></div>`)};
window.setFocusDuration=()=>{let n=Math.max(1,Number(document.querySelector('#focusamount')?.value||25)),u=document.querySelector('#focusunit')?.value||'minutes';let mult=u==='hours'?3600:u==='seconds'?1:60;focusTotal=Math.round(n*mult);focusLeft=focusTotal;let el=document.querySelector('#focusclock');if(el)el.textContent=formatFocus(focusLeft)};
window.startFocus=()=>{if(focusTimer)return;if(!focusLeft)setFocusDuration();focusTimer=setInterval(()=>{focusLeft--;let el=document.querySelector('#focusclock');if(el)el.textContent=formatFocus(focusLeft);if(focusLeft<=0){clearInterval(focusTimer);focusTimer=null;alert('Focus session complete! Take a 5-minute break 🎉');}},1000)};
window.resetFocus=()=>{clearInterval(focusTimer);focusTimer=null;focusLeft=focusTotal;let el=document.querySelector('#focusclock');if(el)el.textContent=formatFocus(focusLeft)};
